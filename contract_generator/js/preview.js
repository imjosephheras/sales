...
(function() {
    ...
    function render(data) {
        ...
        // Product mode: render parts replacement invoice instead of normal templates
        if (data._salesMode === 'product') {
            let html = renderPartsReplacementInvoice(data);
            setTimeout(() => { previewContent.innerHTML = html; }, 100);
            return;
        }
        ...
        return `
            <div class="document-preview jwo-preview-exact">
                ...
                <table class="jwo-services-exact">
                    <thead>
                        <tr>
                            <th style="width: 25%;">${data._salesMode === 'product' ? 'PRODUCT' : 'TYPE OF SERVICES'}</th>
                            <th style="width: 12%;">${data._salesMode === 'product' ? 'QUANTITY' : 'SERVICE TIME'}</th>
                            <th style="width: 12%;">${data._salesMode === 'product' ? 'UNIT PRICE' : 'FREQUENCY'}</th>
                            <th style="width: 36%;">${data._salesMode === 'product' ? 'DESCRIPTION' : 'SERVICE DESCRIPTION'}</th>
    ...
    // ========================================
    // PARTS REPLACEMENT INVOICE RENDERER
    // ========================================

    /**
     * renderPartsReplacementInvoice(data)
     * Renders an independent invoice-style document for parts replacement
     * when the editor is in product mode.
     */
    function renderPartsReplacementInvoice(data) {
        // Determine logo
        var dept = (data.Service_Type || '').toLowerCase();
        var logoSrc = dept.indexOf('hospitality') !== -1
            ? 'Images/Hospitality.png'
            : 'Images/Facility.png';

        // Client info
        var clientName = data.client_name || data.Client_Name || '';
        var companyName = data.Company_Name || '';
        var companyAddress = data.Company_Address || '';
        var clientPhone = data.Number_Phone || '';
        var clientEmail = data.Email || '';
        var seller = data.Seller || '';
        var woNumber = data.docnum || 'DRAFT';

        // Collect product rows from all service data
        var productRows = [];
        var sources = [
            data.janitorial_services,
            data.kitchen_services,
            data.hood_vent_services
        ];
        sources.forEach(function(arr) {
            if (arr && Array.isArray(arr)) {
                arr.forEach(function(svc) {
                    productRows.push(svc);
                });
            }
        });

        // Build product rows HTML and calculate totals
        var totalAmount = 0;
        var productRowsHtml = '';

        if (productRows.length === 0) {
            productRowsHtml = '<tr><td colspan="6" style="text-align:center; padding:20px; color:#999;">No products added yet</td></tr>';
        } else {
            productRows.forEach(function(row, idx) {
                var productName = row.service_type || '';
                var qty = parseFloat(row.service_time) || 0;
                var unitPrice = parseFloat(row.frequency) || 0;
                var rowSubtotal = parseFloat(row.subtotal) || 0;
                // If subtotal is 0 but qty and price exist, calculate
                if (rowSubtotal === 0 && qty > 0 && unitPrice > 0) {
                ...
                productRowsHtml += '<tr>' +
                    ...
                    '<td style="text-align:center;">' + (qty > 0 ? qty : '') + '</td>' +
                    '<td style="text-align:right;">' + (unitPrice > 0 ? '$' + formatPrice(unitPrice) : '') + '</td>' +
                    '<td style="text-align:right; font-weight:bold;">$' + formatPrice(rowSubtotal) + '</td>' +
                    '<td style="text-align:left;">' + escapeHtml(desc) + '</td>' +
                    '</tr>';
            });
        }

        var taxes = totalAmount * 0.0825;
        var grandTotal = totalAmount + taxes;

        return `
            <div class="document-preview" style="font-family: Arial, Helvetica, sans-serif; font-size: 10pt; color: #000; padding: 20px;">
                <style>
                    .pri-header { display: flex; align-items: center; border-bottom: 3px solid #CC0000; padding-bottom: 10px; margin-bottom: 15px; }
                    .pri-header img { max-height: 60px; margin-right: 15px; }
                    .pri-header .pri-title-block { text-align: right; flex: 1; }
                    .pri-header .pri-title { color: #CC0000; font-size: 16pt; font-weight: bold; text-transform: uppercase; }
                    .pri-header .pri-subtitle { font-size: 9pt; color: #333; font-style: italic; }

                    .pri-info-table { width: 100%; border-collapse: collapse; margin-bottom: 15px; font-size: 9pt; }
                    .pri-info-table td { padding: 4px 8px; vertical-align: top; }
                    .pri-info-table .pri-label { font-weight: bold; color: #CC0000; width: 22%; }
                    .pri-info-table .pri-value { width: 28%; }

                    .pri-desc-box { background-color: #f8f8f8; border-left: 4px solid #CC0000; padding: 10px 14px; margin-bottom: 15px; font-size: 9pt; line-height: 1.5; }
                    .pri-desc-box strong { color: #CC0000; }

                    .pri-products-table { width: 100%; border-collapse: collapse; margin-bottom: 10px; }
                    .pri-products-table th { background-color: #CC0000; color: white; font-weight: bold; padding: 8px 6px; text-align: center; border: 1px solid #000; font-size: 8pt; text-transform: uppercase; }
                    .pri-products-table td { border: 1px solid #ccc; padding: 6px 8px; font-size: 8pt; }
                    .pri-products-table tr:nth-child(even) td { background-color: #f9f9f9; }

                    .pri-totals-table { width: 280px; border-collapse: collapse; margin-left: auto; margin-bottom: 20px; }
                    .pri-totals-table td { padding: 6px 8px; font-size: 9pt; }
                    .pri-totals-table .pri-tot-label { text-align: left; font-weight: bold; text-transform: uppercase; }
                    .pri-totals-table .pri-tot-value { text-align: right; font-weight: bold; border: 1px solid #000; }
                    .pri-totals-table tr:last-child .pri-tot-label { color: #CC0000; }
                    .pri-totals-table tr:last-child .pri-tot-value { background-color: #CC0000; color: white; border-color: #CC0000; }

                    .pri-sig-section { margin-top: 30px; }
                    .pri-sig-title { color: #CC0000; font-weight: bold; font-size: 10pt; text-transform: uppercase; margin-bottom: 15px; border-bottom: 2px solid #CC0000; padding-bottom: 4px; }
                    .pri-sig-grid { display: flex; gap: 20px; }
                    .pri-sig-col { flex: 1; }
                    .pri-sig-box { border: 1px solid #000; padding: 10px; margin-bottom: 10px; height: 70px; }
                    .pri-sig-label { font-weight: bold; font-size: 9pt; text-transform: uppercase; margin-bottom: 5px; }
                    .pri-sig-line { border-top: 1px solid #000; margin-top: 30px; padding-top: 3px; font-size: 8pt; }

                    .pri-footer { margin-top: 20px; }
                    .pri-footer-top { background-color: #A30000; color: white; text-align: center; padding: 3px 10px; font-size: 7pt; }
                    .pri-footer-bottom { background-color: #CC0000; color: white; text-align: center; padding: 8px 10px; font-size: 8pt; }
                </style>

                <!-- HEADER -->
                <div class="pri-header">
                    <img src="${logoSrc}" alt="Logo" onerror="this.style.display='none'">
                    <div class="pri-title-block">
                        <div class="pri-title">Factura por Reemplazo de Piezas</div>
                        <div class="pri-subtitle">Parts Replacement Invoice</div>
                    </div>
                </div>

                <!-- CLIENT INFO -->
                <table class="pri-info-table">
                    <tr>
                        <td class="pri-label">W.O. No.:</td>
                        <td class="pri-value">${escapeHtml(woNumber)}</td>
                        <td class="pri-label">Date:</td>
                        <td class="pri-value">${new Date().toLocaleDateString('en-US')}</td>
                    </tr>
                    <tr>
                        <td class="pri-label">Client / Company:</td>
                        <td class="pri-value">${escapeHtml(companyName)}</td>
                        <td class="pri-label">Contact:</td>
                        <td class="pri-value">${escapeHtml(clientName)}</td>
                    </tr>
                    <tr>
                        <td class="pri-label">Address:</td>
                        <td class="pri-value">${escapeHtml(companyAddress)}</td>
                        <td class="pri-label">Phone:</td>
                        <td class="pri-value">${escapeHtml(clientPhone)}</td>
                    </tr>
                    <tr>
                        <td class="pri-label">Sales Person:</td>
                        <td class="pri-value">${escapeHtml(seller)}</td>
                        <td class="pri-label">Email:</td>
                        <td class="pri-value">${escapeHtml(clientEmail)}</td>
                    </tr>
                </table>

                <!-- DESCRIPTION -->
                <div class="pri-desc-box">
                ...
                <!-- PRODUCTS TABLE -->
                <table class="pri-products-table">
                    <thead>
                        <tr>
                            <th style="width:5%;">#</th>
                ...
                <!-- TOTALS -->
                <table class="pri-totals-table">
                    <tr>
                        <td class="pri-tot-label">TOTAL</td>
                        <td class="pri-tot-value">$${formatPrice(totalAmount)}</td>
                    </tr>
                    <tr>
                        <td class="pri-tot-label">TAXES (8.25%)</td>
                        <td class="pri-tot-value">$${formatPrice(taxes)}</td>
                    </tr>
                    <tr>
                        <td class="pri-tot-label">GRAND TOTAL</td>
                        <td class="pri-tot-value">$${formatPrice(grandTotal)}</td>
                    </tr>
                </table>

                ${data.Additional_Comments ? `
                <div class="pri-desc-box" style="margin-top: 10px;">
                    <strong>Additional Notes:</strong><br>
                    ${escapeHtml(data.Additional_Comments).replace(/\n/g, '<br>')}
                </div>` : ''}

                <!-- SIGNATURES -->
                <div class="pri-sig-section">
                    <div class="pri-sig-title">Signatures / Firmas</div>
                    <div class="pri-sig-grid">
                        <div class="pri-sig-col">
                            <div class="pri-sig-box">
                                <div class="pri-sig-label">Authorized by / Autorizado por:</div>
                                <div class="pri-sig-line">Signature & Date / Firma y Fecha</div>
                            </div>
                            <div class="pri-sig-box">
                                <div class="pri-sig-label">Print Name / Nombre:</div>
                                <div class="pri-sig-line">Name & Title / Nombre y Cargo</div>
                            </div>
                        </div>
                        <div class="pri-sig-col">
                            <div class="pri-sig-box">
                                <div class="pri-sig-label">Received by / Recibido por:</div>
                                <div class="pri-sig-line">Signature & Date / Firma y Fecha</div>
                            </div>
                            <div class="pri-sig-box">
                                <div class="pri-sig-label">Print Name / Nombre:</div>
                ...
                <div class="pri-footer">
                    <div class="pri-footer-top">PRIME FACILITY SERVICES GROUP, INC.</div>
                    <div class="pri-footer-bottom">
                        <strong>8303 Westglen Dr - Houston, TX 77063 - Phone 713-338-2553 - Fax 713-574-3065</strong><br>
                        www.primefacilityservicesgroup.com
                    </div>
                </div>
